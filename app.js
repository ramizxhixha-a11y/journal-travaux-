...
/* ───── CONSTANTES ─────────────────────────────────────────────────── */
const APP_VERSION = '2.3bis';
// V2.3bis : trim() défensif sur owner/repo/branche/token GitHub
//   - nettoyage auto à l'ouverture (legacy data avec espace fantôme)
//   - trim au save Réglages
//   - trim avant chaque appel API (testConnection, publish, downloadPDF)
// Évite l'erreur 404 quand l'autocorrect Android ajoute un espace à un champ.
...
async function loadConfig() {
  ...
  // V2.3bis — nettoie les espaces fantômes sur les champs GitHub (legacy data)
  ['gh_owner', 'gh_repo', 'gh_branch', 'gh_token'].forEach(k => {
    if (typeof cfg[k] === 'string') cfg[k] = cfg[k].trim();
  });
  state.config = cfg;
  return cfg;
}
...
function ghConfigured() {
  // V2.3bis — trim défensif
  const owner = (state.config.gh_owner || '').trim();
  const repo  = (state.config.gh_repo  || '').trim();
  const token = (state.config.gh_token || '').trim();
...
async function testGitHubConnection() {
  readForm($('#screen-reglages'), { config: state.config });
  // V2.3bis — trim défensif (anti espaces fantômes par autocorrect/copier-coller)
  const c = {
    gh_owner:  (state.config.gh_owner  || '').trim(),
    gh_repo:   (state.config.gh_repo   || '').trim(),
    gh_branch: (state.config.gh_branch || '').trim() || 'main',
    gh_token:  (state.config.gh_token  || '').trim(),
  };
...
async function publishToGitHub() {
  ...
  const projet_id = state.config.projet_id;
...
function wireEvents() {
  ...
  $('#btn-save-config').onclick = async () => {
    ...
    readForm($('#screen-reglages'), { config: state.config });
    // V2.3bis — trim défensif sur les champs GitHub avant persistance
    ['gh_owner', 'gh_repo', 'gh_branch', 'gh_token'].forEach(k => {
      if (typeof state.config[k] === 'string') state.config[k] = state.config[k].trim();
    });
    ...
    // V2.3bis — refresh le form avec les valeurs trimées (visuellement propre)
    fillForm($('#screen-reglages'), { config: state.config });
    await loadProjet();
    await loadAll();
    renderAccueil();
