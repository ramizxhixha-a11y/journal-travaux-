...
// PRÉSETS DE PROJETS — données pré-remplies pour les chantiers connus
const PROJECT_PRESETS = {
  'PRJ-CHIREC-CAVELL': {
    nom: 'CHIREC Cavell — Extension hôpital de jour chirurgical',
    ministere: 'Centre Hospitalier Interrégional Edith Cavell (CHIREC)',
    administration: 'Direction des infrastructures',
    service: 'Suivi technique des marchés publics — infrastructures hospitalières',
    dossier_no: 'CHIREC-CAVELL-2026-LOT3',
    travaux_de: 'Extension hôpital de jour chirurgical — Lot 3 (HVAC + Sanitaire)',
    cahier_charges_no: '2026-CHIREC-CAVELL-L3',
    cahier_charges_de: 'Cahier spécial des charges CHIREC Cavell Lot 3',
    maitre_oeuvre: {
      nom: 'CHIREC — Direction des infrastructures',
      adresse: '37 rue Général Lotz, 1180 Uccle',
      contact: 'M. Pierre Lambert (Fonctionnaire dirigeant)',
    },
    bureau_etude: {
      nom: 'Sweco Belgium',
      adresse: '',
    ...
    adresse_chantier: '37 rue Général Lotz, 1180 Uccle',
    fonctionnaire_dirigeant: 'M. Pierre Lambert',
    prepose_surveillance: 'M. Jean-Marie Herbots',
  },
};
...
function emptyProjet(id) {
  ...
  // Appliquer le préset si l'ID est connu
  const preset = PROJECT_PRESETS[id];
  if (preset) {
    Object.assign(base, preset);
    base.entrepreneur = { ...base.entrepreneur, ...(preset.entrepreneur || {}) };
    base.maitre_oeuvre = { ...base.maitre_oeuvre, ...(preset.maitre_oeuvre || {}) };
    base.bureau_etude = { ...base.bureau_etude, ...(preset.bureau_etude || {}) };
  }
  return base;
}
...
function emptyComplement(folioRef = null) {
  return {
    id: uid(),
    projet_id: state.config.projet_id,
    folio_no: nextComplementNo(folioRef),   // V2.2 : même N° que le folio
    folio_no_ref: folioRef ? folioRef.folio_no : null,
    date_ref: folioRef ? folioRef.date : todayISO(),
    sections: {},   // V2.2 : { "B": "texte suite B...", "J": "texte suite J...", ... }
    // compat V2.1 : champ legacy
    case: '',
    texte: '',
    signature_prepose: '',
    signature_entrepreneur: '',
...
async function saveComplement() {
  ...
  // V2.2 : lire les sections depuis le DOM avant de sauvegarder
...
function complementItemHTML(c) {
  // V2.2 : afficher les cases présentes
  ...
  return `
    <li class="folio-item" data-id="${c.id}">
      ...
      <div class="folio-info">
        <div class="folio-date">Cases ${casesList} · folio ${c.folio_no_ref || '?'}</div>
        <div class="folio-summary">${escapeHTML(sum)}</div>
      </div>
    </li>`;
}
...
/* ═════════════════════════════════════════════════════════════════════
   COMPLÉMENT V2.2 — rendu multi-sections
   ═════════════════════════════════════════════════════════════════════ */
...
function renderComplementScreen(c) {
  ...
  addDiv.innerHTML = `
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      ${Object.keys(CASE_LABELS).filter(k => !sections[k]).map(k =>
        `<button type="button" class="btn-add-section" data-addcase="${k}">${k}</button>`
      ).join('')}
    </div>`;
  ...
  // Réactiver spellcheck et autosave sur les nouveaux éléments
  enableSpellcheck(sectionsEl);
  $$('textarea', sectionsEl).forEach(ta => {
    ta.addEventListener('input', () => scheduleAutoSave('complement'));
  });

  // Signatures
  const sigsEl = $('#complement-sigs');
  if (sigsEl) {
    sigsEl.querySelector('[data-field="complement.signature_prepose"]')?.setValue
...
function makeSectionBlock(c, letter, texte) {
  ...
  div.innerHTML = `
    <div class="complement-section-header">
      ...
      <span class="complement-section-label">Suite de la case ${letter} — ${CASE_LABELS[letter] || ''}</span>
...
function readComplementSections(c) {
  // Lire les textareas de sections et mettre à jour c.sections
  if (!c.sections) c.sections = {};
...
function openComplement(c, targetCase = null) {
  ...
  // V2.2 : si une case cible est précisée, scroll vers elle
  if (targetCase) {
    setTimeout(() => {
...
async function createNewComplement(folioRef = null, caseLetter = null) {
  ...
  // V2.2 : si une case est précisée et la section n'existe pas, la créer à la volée
  if (caseLetter) {
    if (!c.sections) c.sections = {};
...
function checkOverflow(textarea) {
  ...
  // badge dans summary (compteur compact)
  ...
  // V2.2 : bouton "suite" uniquement cases C-K (jamais A ni B)
  const casesCK = ['C','D','E','F','G','H','J','K'];
  if (!casesCK.includes(caseLetter)) {
    // Cases A et B : pas de bouton "suite", elles s'agrandissent librement
    return over;
  }

  // Bouton "suite" dans la summary (court, orange)
...
async function pickProject() {
  // Lister tous les projets stockés
  const allProjects = await dbGetAll(STORES.projet);
  if (!allProjects.length) {
    const create = confirm('Aucun projet enregistré. Créer un nouveau projet maintenant ?');
    if (create) await newProject();
    return;
  }
  // Construire la liste pour modal
  const items = allProjects.map(p => ({
    id: p.id,
    label: `${p.id}${p.nom && p.nom !== p.id ? ' — ' + p.nom : ''}`,
    current: p.id === state.config.projet_id,
  }));
  showProjectPicker(items);
}

function showProjectPicker(items) {
  // Modal de sélection
  const existing = document.getElementById('project-picker-modal');
  ...
  modal.innerHTML = `
    <div class="modal-box">
      ...
      <ul class="project-list">
        ${items.map(it => `
          <li class="project-item${it.current ? ' current' : ''}" data-id="${it.id}">
            <span class="project-label">${escapeHTML(it.label)}</span>
            ${it.current ? '<span class="project-badge">en cours</span>' : ''}
            <button class="btn-mini-del" data-del="${it.id}" title="Supprimer">×</button>
          </li>
        `).join('')}
      </ul>
      <div class="modal-actions">
        <button class="btn btn-primary" id="modal-new-project">+ Nouveau projet</button>
        <button class="btn" id="modal-close">Annuler</button>
      </div>
    </div>`;
  document.body.appendChild(modal);

  // Wire selection
  modal.querySelectorAll('.project-item').forEach(li => {
    li.addEventListener('click', async (e) => {
      if (e.target.classList.contains('btn-mini-del')) return;
      const id = li.dataset.id;
      await saveConfigKey('projet_id', id);
      await loadProjet();
      await loadAll();
      renderAccueil();
      modal.remove();
  ...
  // Wire delete
  modal.querySelectorAll('.btn-mini-del').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const id = btn.dataset.del;
      if (!confirm(`Supprimer le projet « ${id} » et toutes ses données ?`)) return;
      await dbDelete(STORES.projet, id);
      // supprimer aussi folios et compléments du projet
      const folios = await dbGetAll(STORES.folios);
      for (const f of folios.filter(x => x.projet_id === id)) await dbDelete(STORES.folios, f.id);
      const comps = await dbGetAll(STORES.complements);
      for (const c of comps.filter(x => x.projet_id === id)) await dbDelete(STORES.complements, c.id);
      if (state.config.projet_id === id) {
        await saveConfigKey('projet_id', '');
        state.projet = null;
        state.folios = [];
        state.complements = [];
        renderAccueil();
      ...
      modal.remove();
      toast(`Projet « ${id} » supprimé`, 'success');
      pickProject();
    });
  });
  modal.querySelector('#modal-new-project').onclick = () => { modal.remove(); newProject(); };
  modal.querySelector('#modal-close').onclick = () => modal.remove();
  modal.addEventListener('click', (e) => { if (e.target === modal) modal.remove(); });
}

async function newProject() {
  const id = prompt('ID du nouveau projet\n(ex : PRJ-CHIREC-CAVELL, PRJ-HOPITAL-XYZ) :');
  if (!id) return;
  const cleanId = id.trim();
  // Vérifier qu'il n'existe pas déjà
  const existing = await dbGet(STORES.projet, cleanId);
  if (existing) {
    const hasPreset = !!PROJECT_PRESETS[cleanId];
    if (hasPreset) {
      const reset = confirm(`Le projet « ${cleanId} » existe déjà.\n\nOK = recharger avec les données pré-remplies (efface tes modifs)\nAnnuler = juste charger sans changer`);
      await saveConfigKey('projet_id', cleanId);
      if (reset) {
        state.projet = emptyProjet(cleanId);
        await saveProjet();
        toast('Données pré-remplies rechargées ✓', 'success');
      } else {
        await loadProjet();
      }
      await loadAll();
      ...
      const load = confirm(`Le projet « ${cleanId} » existe déjà. Le charger ?`);
      if (load) {
        await saveConfigKey('projet_id', cleanId);
        await loadProjet();
        await loadAll();
  ...
  await saveConfigKey('projet_id', cleanId);
  state.projet = emptyProjet(cleanId);  // applique preset si CHIREC
  await saveProjet();
  state.folios = [];
  state.complements = [];
  renderAccueil();
  ...
  const hasPreset = !!PROJECT_PRESETS[cleanId];
  toast(hasPreset
    ? `Projet « ${cleanId} » créé — données pré-remplies ✓`
    : `Projet « ${cleanId} » créé — remplis les infos ci-dessous`, 'success');
}
...
(async function init() {
  try {
    ...
    console.log('[Journal V2.2] prêt — projet:', state.config.projet_id || '(aucun)');
  } catch (e) {
